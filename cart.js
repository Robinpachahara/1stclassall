...
// First, create a module-level variable for the cart instance
...
class Cart {
    ...
    async submitToSheet() {
        ...
        if (!phoneNumber || !/^\d{10}$/.test(phoneNumber)) {
            this.notifications.show('Please enter a valid 10-digit phone number', 'error');
            return;
        }
        ...
        try {
            ...
            this.notifications.show('Your order has been submitted successfully. We will contact you shortly.');
            // Removed the line that closes the form
            // document.getElementById('dropdownForm').style.display = 'none';
            
        } catch (error) {
            console.error('Error submitting order:', error);
            this.notifications.show('There was an error submitting your order. Please try again or contact us directly.', 'error');
        }
    }
}
...
// Function to get or create cart instance
function getCart() {
    if (!cartInstance) {
...
// Export cart instance getter to window
...
// Set up initialization based on document readiness
if (document.readyState === 'complete') {
    initializeCart();
} else {
...
// Export submitCartToSheet function
window.submitCartToSheet = function() {
    getCart().submitToSheet();
};